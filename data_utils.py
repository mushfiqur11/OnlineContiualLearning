# This part of file is fetched from pulic soucecode : "https://github.com/arslan-chaudhry/orthog_subspace/blob/master/utils/data_utils.py"
...
def construct_split_miniImagenet(task_labels, data_file):
    ...
    for task in task_labels:
        ...
        train= DataSet(deepcopy(x_train), deepcopy(y_train), dtype=dtypes.uint8, reshape=False)
        test= DataSet(deepcopy(x_test), deepcopy(y_test), dtype=dtypes.uint8, reshape=False)
        datasets.append(base.Datasets(train=train, validation=[], test=test))
        # train = {
